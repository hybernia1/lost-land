...
| done | Environment | Implementovat nahodne prechody `stable`, `rain`, `snowFront`, `radiation`. | `npm run build` prosel. | Deterministicke podle save id a casu. |
...
| done | Radiace | Pridat prvni gameplay dopad radiace. | `npm run build` prosel. | Zvysuje incidenty a ubira moralku. |
...
| done | Rendering | Pridat jednoduchy overlay/tint pro rain/snow/radiation. | `npm run build` prosel. | Placeholder Pixi efekty. |
